...
DH 3.3.2 和 SSRD 1.8.7 对本项目而言是可选的渲染模组。[源码与 JAR 审计](docs/DH-SSRD-3.3.2-1.8.7-audit.md)没有发现此次更新需要改动地表兼容代码或依赖声明的位置。维护者反馈，更新后的模组组合在本地 JJ2096 实例中运行正常。这是维护者对该实例的验证反馈，不代表所有渲染场景已经测完，也不是对本项目公开 beta JAR 的测试记录。
